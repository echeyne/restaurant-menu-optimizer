...
class PendingMenuOptimizationsScreen extends StatefulWidget {
  const PendingMenuOptimizationsScreen({super.key});

  @override
  State<PendingMenuOptimizationsScreen> createState() =>
      _PendingMenuOptimizationsScreenState();
}

class _PendingMenuOptimizationsScreenState
    extends State<PendingMenuOptimizationsScreen> with RestaurantLoaderMixin {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _loadPendingOptimizations();
    });
  }

  Future<void> _loadPendingOptimizations() async {
  ...
  Future<void> _refreshOptimizations() async {
    setState(() {
      _isRefreshing = true;
    });

    try {
      await _loadPendingOptimizations();
    } finally {
      setState(() {
        _isRefreshing = false;
      });
  ...
  void _viewOptimization(OptimizedMenuItem item) {
    Navigator.push(
      context,
      MaterialPageRoute(
    ...
    return Consumer2<MenuProvider, RestaurantProvider>(
      builder: (context, menuProvider, restaurantProvider, child) {
        return withRestaurantLoading(() {
          final pendingOptimizations = menuProvider.optimizedItems
              .where((item) => item.status == 'pending')
              .toList();

          return Scaffold(
            appBar: AppBar(
              title: const Text('Pending Menu Optimizations'),
              leading: IconButton(
              ...
              actions: [
                IconButton(
                  onPressed: _isRefreshing ? null : _refreshOptimizations,
                  icon: _isRefreshing
                      ? const SizedBox(
                          width: 20,
                          height: 20,
            ...
            body: menuProvider.isLoading
                ...
                : menuProvider.error != null
                    ...
                    : pendingOptimizations.isEmpty
                        ? _buildEmptyState()
                        : _buildOptimizationsList(pendingOptimizations),
          );
        });
      },
    );
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: _refreshOptimizations,
  ...
  Widget _buildEmptyState() {
    // Check if there are any optimizations at all (including completed ones)
    final menuProvider = Provider.of<MenuProvider>(context, listen: false);
    final allOptimizations = menuProvider.optimizedItems;

    if (allOptimizations.isEmpty) {
      // Show loading state if no optimizations exist yet
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
      ...
      // Show empty state for no pending optimizations
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            ...
            const SizedBox(height: 12),
            Text(
              'All your menu optimizations have been processed.\nCheck completed optimizations or start a new optimization.',
              textAlign: TextAlign.center,
              style: TextStyle(
      ...
      children: [
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              Text(
                '${optimizations.length} Pending Optimization${optimizations.length == 1 ? '' : 's'}',
                style: TextStyle(
                  color: Colors.blue[700],
                  fontWeight: FontWeight.bold,
                  fontSize: 16,
        ...
        Expanded(
          child: RefreshIndicator(
            onRefresh: _refreshOptimizations,
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return OptimizationItemCard(
                  item: optimization,
                  onTap: () => _viewOptimization(optimization),
                  showStatus: true,
                  isCompact: false,
                );
              },
